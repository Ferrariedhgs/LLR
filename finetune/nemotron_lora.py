"""
Modal LoRA fine-tuning for nvidia/Nemotron-Mini-4B-Instruct (nemotron-nano-4b)
on a folder of JSONL datasets, then exports to GGUF in fp16, q8_0, q4_K_M.

Usage:
    modal run train_nemotron_lora.py                          # full run
    modal run train_nemotron_lora.py --data-dir ./my_data     # custom data folder
    modal run train_nemotron_lora.py --dry-run                # validate dataset only
...
Outputs written to ./output/ (local) after the run:
  nemotron-room-lora-fp16.gguf
  nemotron-room-lora-q8_0.gguf
  nemotron-room-lora-q4_K_M.gguf
"""
...
# The nvidia/ model uses a custom Mamba-H architecture requiring trust_remote_code.
# unsloth/NVIDIA-Nemotron-3-Nano-4B is the unsloth-patched mirror that loads cleanly.
BASE_MODEL = "unsloth/NVIDIA-Nemotron-3-Nano-4B"
VOLUME_NAME = "nemotron-lora-vol"

# Persistent volume to cache the base model weights between runs
volume = modal.Volume.from_name(VOLUME_NAME, create_if_missing=True)

# Modal A100 driver: 580.x → CUDA 13.0 → need cu130 torch wheels.
# causal-conv1d and mamba-ssm require nvcc to compile, so we use the
# nvidia devel base which has the full CUDA toolkit including nvcc.
# torch is installed last with --force-reinstall to prevent unsloth/mamba
# from pulling 2.12 back in.
image = (
    ...
    .apt_install(
        ...
        "libgomp1", "ninja-build", "clang",
    )
    .run_commands(
    "pip install --upgrade pip setuptools wheel",
    )
    # Step 1: numpy + packaging needed before torch and mamba compile steps.
    .pip_install("numpy", "packaging")
    # Step 2: torch cu130 — must exist before mamba-ssm reads torch version.
    .pip_install(
        "torch==2.11.0",
        "torchvision==0.26.0",
        index_url="https://download.pytorch.org/whl/cu130",
    )
    # Step 3: causal-conv1d then mamba-ssm — compiled with nvcc + torch present.
    # --no-build-isolation prevents pip from creating an isolated build env
    # that can't see the torch we just installed in the system site-packages.
    .run_commands(
        "pip install --no-build-isolation causal-conv1d>=1.4.0",
        "pip install --no-build-isolation mamba-ssm",
    )
    # Step 4: rest of training stack. Torch already locked, resolver skips it.
    .pip_install(
        "trl>=0.8.6",
        "huggingface_hub",
        "datasets",
    ...
    # Step 5: unsloth last — sees torch already satisfied, won't upgrade it.
    .pip_install(
        "unsloth[colab-new] @ git+https://github.com/unslothai/unsloth.git",
    )
    # Step 6: hard-pin torch — final insurance against any stray upgrade.
    .run_commands(
        "pip install --force-reinstall --no-deps torch==2.11.0+cu130"
...
app = modal.App("nemotron-lora-train-v5", image=image)
...
def format_chat_prompt(record: dict, tokenizer) -> str:
    """Format as a Nemotron chat template (system + user + assistant)."""
    messages = [
...
def train(
    ...
    fp16_gguf  = os.path.join(gguf_dir, "nemotron-room-lora-fp16.gguf")
    q8_gguf    = os.path.join(gguf_dir, "nemotron-room-lora-q8_0.gguf")
    q4km_gguf  = os.path.join(gguf_dir, "nemotron-room-lora-q4_K_M.gguf")
...
def debug_env():
    """Standalone diagnostic — run with: modal run nemotron_lora.py::debug_env"""
    import subprocess, torch
    print(f"torch version  : {torch.__version__}")
    print(f"cuda available : {torch.cuda.is_available()}")
    print(f"cuda version   : {torch.version.cuda}")
...
def main(
    ...
    print("  nemotron-room-lora-fp16.gguf")
    print("  nemotron-room-lora-q8_0.gguf")
    print("  nemotron-room-lora-q4_K_M.gguf")
